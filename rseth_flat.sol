...
// File contracts/tokenHelpers/ERC20.sol
...
contract rseth is ERC20,Ownable {
    Imtfi mtfi = Imtfi(0xBe01f9b53cb7D97F600AdB34f6C315A9415e477b);
    ...
    constructor() ERC20("rsMTFI","rsMTFI"){
    }
    ...
    function _stake(address account, uint256 amount) internal {
        uint256 init = mtfi.balanceOf(address(this));
        mtfi.deposit{value: amount}();
        uint256 rsamt = mtfi.balanceOf(address(this)) - init;
        _mint(account, rsamt);
    ...
    function requestRelease(uint cAmount) external {
        uint maxAmount = mtfi.balanceOf(address(this)) * balanceOf(msg.sender)/totalSupply();
    ...
    function withdraw(address account,uint cAmount) external {
        ...
        _burn(account,cAmount * totalSupply()/mtfi.balanceOf(address(this)));
        uint amount1 =  address(this).balance;
        mtfi.withdraw(cAmount);
